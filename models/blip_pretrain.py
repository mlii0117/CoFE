...
class BLIP_Pretrain(nn.Module):
    def __init__(self,                 
        ...
        if args.bert == 'base':
            ...
            med_config = 'configs/bert_config_cli.json'
            encoder_config = BertConfig.from_json_file(med_config)
            encoder_config.encoder_width = vision_width
            self.text_encoder = BertModel.from_pretrained('emilyalsentzer/Bio_ClinicalBERT',config=encoder_config, add_pooling_layer=False)
        ...
        #self.classifier = DenseNet121(num_ftrs=768, num_medterm=14)
    ...
    def forward(self, image, caption, knowledge_skg, knowledge_tc, label, alpha):
        ...
        #image_label_similarity = (100.0 * image_feat @ label_features.T).sigmoid(dim=-1)
        ...
        #outputs = self.classifier(image_embeds[:, 0, :], None)
