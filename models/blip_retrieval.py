...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, knowledge_skg, knowledge_tc, alpha, idx):
        with torch.no_grad():
        ...
        if self.args.have_know:
            ...
            text_knowledge_queue, knowledge_input_ids_queue, knowledge_attention_mask_queue = None, None, None
        ...
        with torch.no_grad():
            ...
            if self.args.SKG_know:
                if self.args.SKG_out == 'image':
                    ...
                    image_embeds_m, _ = self.cross_attn_m(tag_output, image_embeds_m)

                image_feat_m = F.normalize(self.vision_proj_m(image_embeds_m[:, 0, :]), dim=-1)

            if self.args.have_know:
                image_embeds_m = self.create_knowledge.get_image_knowledge(image.device, image_feat_m, image_embeds_m, k=3)
                image_feat_m = F.normalize(self.vision_proj(image_embeds_m[:, 0, :]), dim=-1)
