...
class BAN_Model(nn.Module):
    def __init__(self, dataset, args, config, tokenizer):
        ...
        if args.ALBEF:
            self.ALBEF = blip_vqa(pretrained=args.pretrained,
                                  args=args,
    ...
    def forward(self, v, q, a, answer_target, q_mask, knowledge, t_queue, k_input_queue, k_mask_queue, split):
        """Forward
        ...
        if self.args.test_C:
            if self.args.vis_feature == 'maml':
                output_close, output_open, w_emb_close, w_emb_open, q_emb_close, q_emb_open, t_queue, k_input_queue, k_mask_queue = \
                    self.ALBEF(knowledge, t_queue, k_input_queue, k_mask_queue, v[0], q, answer_target, q_mask=q_mask, split=split)
            elif self.args.vis_feature == 'ae':
                output_close, output_open, w_emb_close, w_emb_open, q_emb_close, q_emb_open, t_queue, k_input_queue, k_mask_queue = \
                    self.ALBEF(knowledge, t_queue, k_input_queue, k_mask_queue, v[1], q, answer_target, q_mask=q_mask, split=split)
            #type_att = self.typeatt(q)
            #a_open, a_close, typeatt_open, typeatt_close = seperate_C(a, type_att, answer_target)
            ...
            if self.args.vis_feature == 'maml':
                cross_modal_embedding = self.ALBEF(knowledge, v[0].repeat(1, 3, 1, 1), q, q_mask=q_mask)
            elif self.args.vis_feature == 'ae':
                cross_modal_embedding = self.ALBEF(knowledge, v[1].repeat(1, 3, 1, 1), q, q_mask=q_mask)
        ...
        return last_output_close, last_output_open, a_close, a_open, t_queue, k_input_queue, k_mask_queue  # test_all
