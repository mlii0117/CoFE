...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        if not self.args.test_best:
            ...
            # for batch_idx, ((images, report, knowledge, labels),(mimic_knowledge))  in enumerate(zip(cycle(self.train_dataloader),self.knowledge_dataloader)):
            for batch_idx, (images, report, knowledge, labels) in enumerate(self.train_dataloader):
                images = images.to(self.device)
                labels = labels.float().to(self.device)
                if self.args.dataset_name == 'chexpert':
                    knowledge = mimic_knowledge

                if self.args.visual == 'BLIP':
                    image_feature = self.model(images, report, knowledge, mode='image', split='train')
        ...
        auc_pre = []
        with torch.no_grad():
            for batch_idx, (images, report, labels, knowledge) in enumerate(self.val_dataloader):
                images, labels = images.to(self.device), labels.float().to(self.device)

                if self.args.visual == 'BLIP':
                    image_feature = self.model(images, None, knowledge, mode='image', split='val')
                    image_feature = image_feature[:, 0, :]  # [bs x 768]
                    outputs = self.classifier(image_feature, images)
        ...
        auc_pre = []
        with torch.no_grad():
            for batch_idx, (images, report, labels, knowledge) in enumerate(self.test_dataloader):
                images, labels = images.to(self.device), labels.float().to(self.device)

                if self.args.visual == 'BLIP':
                    image_feature = self.model(images, None, knowledge, mode='image', split='test')
                    image_feature = image_feature[:, 0, :]  # [bs x 768]
                    outputs = self.classifier(image_feature, images)
