...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        if not self.args.test_best:
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.train_dataloader):
                ...
                loss = self.model(images, reports_ids, reports_masks)
    ...
    def _train_epoch_blip(self, epoch):
        ...
        if not self.args.test_best:
            self.model.train()
            for batch_idx, (images, captions, knowledge_skg, knowledge_tc, label) in enumerate(self.train_dataloader):
                images = images.to(self.device)
                ...
                loss = self.model(images, captions, alpha, label)
                train_loss += loss.item()
                self.writer.add_scalar("data/Loss", loss.item(), batch_idx + len(self.train_dataloader) * (epoch - 1))
                # To activate the tensorboard: tensorboard --logdir=runs --bind_all
                print_loss += loss.item()
        ...
        if self.args.test_best:
            self._resume_checkpoint(self.args.checkpoint)
        self.model.eval()
        with torch.no_grad():
            val_gts, val_res = [], []
            a = (0, 0, 0, 0)
