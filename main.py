...
import numpy as np
from generation_api.dataloaders_blip import R2DataLoader
...
def main(args, config):
    # parse arguments
    # fix random seeds
    # print(os.path.join(args.save_dir, 'model_best.pth'))
    print(args.task)
    print(args.dataset_name)
    print('--------------------visual-------------------------')
    print(args.test_visual)
    print('---------------------text--------------------------')
    print(args.test_text)
    print('-------------------have_know----------------------')
    print(args.have_know)
    ...
    elif args.tokenizer == 'blip':
        if args.bert == 'base':
            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        elif args.bert == 'sci':
            tokenizer = AutoTokenizer.from_pretrained(r"./project/dataset/scibert_scivocab_uncase/")
        elif args.bert == 'cli':
    ...
    # model = MRGModel(args, tokenizer,config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_dir', type=str,
                        default='./project/dataset/MIMIC-CXR/mimic_cxr/images',
                        help='the path to the directory containing the data.')
    parser.add_argument('--ann_path', type=str,
                        default='./project/dataset/MIMIC-CXR/mimic_cxr/annotation.json',
                        help='the path to the directory containing the data.')
    parser.add_argument('--knowledge_path', type=str,
                        default='./project/dataset/iu_xray/iu_xray_kg_label.json&./dataset/mimic/mimic_train_kl_small.json',
                        help='the path to the directory containing the data.')  # mimic_train_kg_label.json mimic_train_kl_small.json

    # Data loader settings
    parser.add_argument('--dataset_name', type=str, default='iu_xray', choices=['cxchr', 'iu_xray', 'mimic_cxr'],
    ...
    parser.add_argument('--tokenizer', type=str, default='blip', choices=['our', 'blip'],
                        help='the dataset to be used.')
    ...
    parser.add_argument('--TI_know', default=False, type=bool)
    parser.add_argument('--concat', default=False, type=bool)
    parser.add_argument('--SKG_out', type=str, default='image', choices=['image', 'tag'],
                        help='the dataset to be used.')
    parser.add_argument('--get_label', default=False, type=bool)
    parser.add_argument('--get_attn', default=False, type=bool)
