...
# from .medical_dataset import generation_train, generation_eval, retrieval_train, retrieval_eval, \
#     diagnose_train, diagnose_eval, diagnose_knowledge  ###
...
def create_dataset(dataset, args, config):
    ...
    if dataset=='pretrain':
        dataset = pretrain_dataset(args.knowledge_path, args.image_dir, transform_train, args=args)
        return dataset
    ...
    if dataset =='generation_iu_xray':
        ...
        # train_dataset = retrieval_train(transform_train, args.image_dir.split('&')[0], args.ann_re.split('&')[0], dataset='iu_xray', args=args)
        # val_dataset = retrieval_eval(transform_test, args.image_dir.split('&')[0], args.ann_re.split('&')[0], 'val', args=args)
        # test_dataset = retrieval_eval(transform_test, args.image_dir.split('&')[0], args.ann_re.split('&')[0], 'test', args=args)
    ...
    elif dataset =='diagnose_iu_xray':
        train_dataset = diagnose_train(transform_train, args.image_dir.split('&')[0], args.knowledge_path.split('&')[0], dataset='iu_xray', args=args)
        val_dataset = diagnose_eval(transform_test, args.image_dir.split('&')[0], args.knowledge_path.split('&')[0], 'val', 'iu_xray', args=args)
        test_dataset = diagnose_eval(transform_test, args.image_dir.split('&')[0], args.knowledge_path.split('&')[0], 'test', 'iu_xray', args=args)
        return train_dataset, val_dataset, test_dataset

    elif dataset =='diagnose_mimic_cxr':
        train_dataset = diagnose_train(transform_train, args.image_dir.split('&')[1], args.knowledge_path.split('&')[1], dataset='mimic_cxr', args=args)
        val_dataset = diagnose_eval(transform_test, args.image_dir.split('&')[1], args.knowledge_path.split('&')[1], 'val', 'mimic_cxr', args=args)
        test_dataset = diagnose_eval(transform_test, args.image_dir.split('&')[1], args.knowledge_path.split('&')[1], 'test', 'mimic_cxr', args=args)
        return train_dataset, val_dataset, test_dataset
